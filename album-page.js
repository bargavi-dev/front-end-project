...
function renderAlbums(albumData){
    ...
    // const url = encodeURI(albumData['artist-credit'][0].name+albumData.title)
        ...
        <button id="d-btn" type="button" class = "btn btn-dark" onclick= 'saveToYourMusic()'>
...
$(window).on("load", function () {
    ...
    function saveToYourMusic(albumId) {

        console.log("added")
        let data = localStorage.getItem("musicLibrary")
        let musicLibrary = JSON.parse(data)
        if(musicLibrary === null) {
            musicLibrary = []
        }
            musicLibrary.push(responseData)
        const musicDataString = JSON.stringify(musicLibrary);
        localStorage.setItem('musicLibrary', musicDataString)
        window.location.href = "your-music.html"
    }
...
// line32   //   <a href="your-music.html" class="card-link"><button id="btn2" type="button" class="btn btn-info">Save to Library</button></a>